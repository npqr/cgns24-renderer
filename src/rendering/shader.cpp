...
Shader::Shader(const char* vertexSource, const char* fragmentSource) {
    create_shader(vertexSource, fragmentSource);
}

void Shader::create_shader(const char* vertexSource, const char* fragmentSource) {
    unsigned int vertex, fragment;
    vertex = compile_shader(vertexSource, VERTEX_SHADER);

    if (check_compile_errors(vertex, VERTEX_SHADER)) {
        return;
    }
    fragment = compile_shader(fragmentSource, FRAGMENT_SHADER);
